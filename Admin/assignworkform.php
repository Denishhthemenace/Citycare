...
<div class="col-sm-5 mt-5 jumbotron">
  ...
  <div class="form-group col-md-6">
    ...
    <select class="form-control" id="assigntech" name="assigntech">
      ...
      <?php
        // Retrieve technician names and categories from the database
        $sql = "SELECT empName, empCat FROM technician_tb";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            echo "<option value='" . $row['empName'] . "'>" . $row['empName'] . " (" . $row['empCat'] . ")</option>";
          }
        }
      ?>
    </select>
